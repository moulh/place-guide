...
public class DateUtils {
	...
	*/
	...
	public static List<String> findDates(String biginDate, String endDate) throws ParseException {
		DateFormat format = new SimpleDateFormat("yyyyMMdd");
		//开始时间
		Calendar calendarBegin = Calendar.getInstance();
		calendarBegin.setTime(format.parse(biginDate));

		//结束时间
		Calendar calendarEnd = Calendar.getInstance();
		calendarEnd.setTime(format.parse(endDate));

		List<String> dateList = new ArrayList<>();
		if(!biginDate.equals(endDate)){
			dateList.add(biginDate);
		}
		while(format.parse(endDate).after(calendarBegin.getTime())){
			calendarBegin.add(Calendar.DAY_OF_MONTH, 1);
			dateList.add(format.format(calendarBegin.getTime()));
		}
		return dateList;
	}
	...
	public static String getWeekOfDate(String date) throws ParseException {
		...
		Date var1 = format.parse(date);
		Calendar calendar = Calendar.getInstance();
		calendar.setTime(var1);
