...
public class RestObjectController {
    ...
    @ApiOperation("分页查询餐饮列表")
    @GetMapping("/list")
    public Result<PageResultDTO<RestObjectListVo>> restList(PageInfo pageInfo,
        ...
        searchMap.put("objectType", FlagConstant.BaseObjectType.TYPE_REST);
        ...
        return Result.ok(objectService.queryRestList(pageInfo, searchMap));
    }

    @ApiOperation("餐饮信息详情")
    @ApiImplicitParam(name = "id", dataType = "integer", value = "逻辑主键id", required = true)
    @GetMapping("/detail/{id}")
    public Result<RestObjectDetailVo> objectDetail(@PathVariable(value = "id") Integer id){
        return Result.ok(objectService.queryRestDetail(id));
    }

    @ApiImplicitParams({
            @ApiImplicitParam(name = "objectId", dataType = "Integer", value = "对象ID,必传", paramType = "query"),
    })
    @ApiOperation("获取该餐饮门店的推荐菜分页信息")
    @GetMapping("/foodinfo/list")
    public Result<PageResultDTO<FoodInfoVo>> queryFoodInfoMsg(PageInfo pageInfo,
                                                     @RequestParam(value = "objectId")Integer objectId){
        Map<String, Object> searchMap = new HashMap<>(6);
        searchMap.put("objectId", objectId);
        return Result.ok(foodInfoService.queryFoodInfoWithImagesList(pageInfo, searchMap));
    }
}
